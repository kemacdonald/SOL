...
acc_ttest <- t.test(mean_accuracy ~ age_group, alternative = "two.sided", 
...
## compute effect size
acc_younger <- ss_kids_df %>% filter(age_group == "< 27 Months") %>% select(mean_accuracy)
acc_older <- ss_kids_df %>% filter(age_group == ">= 27 Months") %>% select(mean_accuracy)

acc_cohd <- round(effsize::cohen.d(acc_older$mean_accuracy, 
                                   acc_younger$mean_accuracy)$estimate, 2)
```
...
ss_kids_df %<>% mutate(mean_acc_logit = logit(mean_accuracy))
...
fit_acc2_prop <- lm(mean_accuracy ~ signs_produced + age_peek_months, data = ss_kids_df)
...
ms_table <- ss_all_df %>% 
    ...
    summarise(mean_acc = mean(mean_accuracy),
              st_dev_acc = sd(mean_accuracy),
              mean_first_shift_acc = mean(C_T_prop, na.rm=T),
              st_dev_first_shift_acc = sd(C_T_prop, na.rm=T))
...
colnames(ms_table) <- c("Age Group", "Accuracy", "SD Accuracy", "Shift Accuracy", "SD Shift Accuracy", "RT", "SD RT")
...
acc_age_plot <- qplot(x = Months, y = mean_accuracy, data = ss_kids_df,
      ylab=c("Accuracy"), 
    ...
    annotate("text", x = 22, y = 0.82, label = paste("r =", acc_age_r), size=8) +
    theme_bw() + 
