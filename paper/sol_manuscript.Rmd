...
author: |
  | Kyle MacDonald^1^, Todd LaMarr^1^, David Corina^2^, Virginia A. Marchman^1^, & Anne Fernald^1^
...
abstract: Research with children learning spoken language shows that the ability to
  ...
  cross-sectional study is the first to explore how young learners of American
  Sign Language (ASL) comprehend signed language in real time, investigating links
...
# some options for pander table output
...
```{r load data}
# load summary data
ss_all_df <- read.csv("sol_ss_all.csv", stringsAsFactors = F)

# filter: just kids
ss_kids_df <- filter(ss_all_df, age_group != "Adults")
...
Figure 1 shows an example of the stimuli and the timeline of one trial in the VLP task. On each trial the child saw two images on the screen for two seconds before the signer appeared. This allowed the child to inspect both images prior to the start of the sentence. Next, children saw a still frame of the signer for one second, which gave them the opportunity to orient to the signer prior to the sentence onset. The target noun was then presented, followed by a question and hold which gave the child the opportunity to their attention to the target object.  After the hold, the signer gave neutral, positive feedback to help maintain the children's focus throughout the task. Each trial lasted for approximately seven seconds. 
...
prop_cd_shifts <- round(shifts_df$count[1] / sum(shifts_df$count), 2) * 100
prop_away_shifts <- round(shifts_df$count[4] / sum(shifts_df$count), 2) * 100
...
```{r correlations age}
# correlations age
cor_acc_age <- cor.test(ss_kids_df$target_looking_signer, ss_kids_df$Months)
...
cor_acc_rt <- cor.test(ss_kids_rt_df$target_looking_signer, ss_kids_rt_df$mean_correct_rt)
...
ms_all <- ss_all_df %>% 
    ...
    summarise(mean_acc = mean(target_looking_signer),
              st_dev_acc = sd(target_looking_signer),
              mean_rt = mean(mean_correct_rt),
              st_dev_rt = sd(mean_correct_rt))
```

```{r t-test rt}
levene_rt <- leveneTest(mean_correct_rt ~ age_group, data=ss_kids_rt_df)
rt_ttest <- t.test(mean_correct_rt ~ age_group, alternative = "two.sided", 
...
## compute effect size
rt_younger <- ss_kids_df %>% filter(age_group == "< 27 Months") %>% select(mean_correct_rt)
rt_older <- ss_kids_df %>% filter(age_group == ">= 27 Months") %>% select(mean_correct_rt)

rt_cohd <- round(effsize::cohen.d(rt_younger$mean_correct_rt,
                                   rt_older$mean_correct_rt)$estimate, 2)
```

```{r t-test acc}
levene_acc <- leveneTest(target_looking_signer ~ age_group, data=ss_kids_df)
acc_ttest <- t.test(target_looking_signer ~ age_group, alternative = "two.sided", 
...
ms_acc <- ss_kids_df %>% 
    group_by(age_group) %>% 
    summarise(mean = mean(target_looking_signer),
              sd = sd(target_looking_signer))

## compute effect size
acc_younger <- ss_kids_df %>% filter(age_group == "< 27 Months") %>% select(target_looking_signer)
acc_older <- ss_kids_df %>% filter(age_group == ">= 27 Months") %>% select(target_looking_signer)

acc_cohd <- round(effsize::cohen.d(acc_older$target_looking_signer, 
                                   acc_younger$target_looking_signer)$estimate, 2)
```

```{r t-test first_shift}
levene_acc_fs <- leveneTest(C_T_prop ~ age_group, data=ss_kids_df)

acc_ttest_fs <- t.test(C_T_prop ~ age_group, alternative = "less", 
                   var.equal=T, data = ss_kids_df)

acc_tval_fs <- round(acc_ttest_fs$statistic, 2)
acc_tdf_fs <- acc_ttest_fs$parameter

## compute effect size
c_t_younger <- ss_kids_df %>% filter(age_group == "< 27 Months") %>% select(C_T_prop)
c_t_older <- ss_kids_df %>% filter(age_group == ">= 27 Months") %>% select(C_T_prop)

coh_d_fs <- round(effsize::cohen.d(c_t_older$C_T_prop, c_t_younger$C_T_prop)$estimate, 2)
```

```{r t-test acc adults vs. kids}
levene_rt_kids_adults <- leveneTest(mean_correct_rt ~ as.factor(age_code), data = ss_all_df)

rt_ttest_kids_adults <- t.test(mean_correct_rt ~ as.factor(age_code), alternative = "less", 
                   var.equal=T, data = ss_all_df)

rt_tval_kids_adults <- round(rt_ttest_kids_adults$statistic, 2)
rt_tdf_kids_adults <- rt_ttest_kids_adults$parameter

## compute effect size
rt_kids <- ss_all_df %>% filter(age_code == "child") %>% select(mean_correct_rt)
rt_adults <- ss_all_df %>% filter(age_code == "adult") %>% select(mean_correct_rt)

coh_d_rt_adults_kids <- round(effsize::cohen.d(rt_kids$mean_correct_rt, 
                                                rt_adults$mean_correct_rt)$estimate, 2)
```

```{r t-test RT adults vs. kids}
levene_acc_kids_adults <- leveneTest(target_looking_signer ~ as.factor(age_code), data = ss_all_df)

acc_ttest_kids_adults <- t.test(target_looking_signer ~ as.factor(age_code), alternative = "greater", 
                   var.equal=T, data = ss_all_df)

acc_tval_kids_adults <- round(acc_ttest_kids_adults$statistic, 2)
acc_tdf_kids_adults <- acc_ttest_kids_adults$parameter

## compute effect size
acc_kids <- ss_all_df %>% filter(age_code == "child") %>% select(target_looking_signer)
acc_adults <- ss_all_df %>% filter(age_code == "adult") %>% select(target_looking_signer)

coh_d_acc_adults_kids <- round(effsize::cohen.d(acc_kids$target_looking_signer, 
                                                acc_adults$target_looking_signer)$estimate, 2)
```

```{r correlations voc and peek}
# correlations vocab
cor_acc_voc <- cor.test(ss_kids_df$target_looking_signer, ss_kids_df$signs_produced)
...
# use the logit to transform mean acc scores from bounded [0-1] to unbounded scale 
ss_kids_df %<>% mutate(mean_acc_logit = logit(target_looking_signer))

fit_acc1 <- lm(target_looking_signer ~ signs_produced, data = ss_kids_df)
fit_acc2 <- lm(target_looking_signer ~ signs_produced + age_peek_months, data = ss_kids_df)
fit_acc3 <- lm(target_looking_signer ~ age_peek_months, data = ss_kids_df)

fit_acc2_prop <- lm(target_looking_signer ~ signs_produced + age_peek_months, data = ss_kids_df)
...
First we present an overview of performance on the VLP task, showing that children become faster and more accurate at comprehending familiar signs as they get older and make progress towards adult levels of language fluency. Then we present multiple regression analyses of the links between children’s real-time ASL processing skills and both age and productive ASL vocabulary. 

### ASL processing efficiency

Figure 2 provides an overview of the timecourse of correct orienting to the referent in response to the target sign. The three curves show changes in the mean proportion of trials on which participants in each age group fixated the correct referent at every 33 ms interval as the target sign unfolded. Before seeing the target sign, all participants fixated on the signer. Interestingly, both adults and children began to increase their looking to the target picture before the offset of the target noun, providing evidence that signers were not waiting until the end of the linguistic utterance to seek out the target image. Children were slower to respond and less accurate than adults, on average, maintaining their gaze on the center signer for approximately `r round(ms_all$mean_rt[1] - ms_all$mean_rt[2])` ms longer and reaching a lower asymptote (Mkids = `r round(ms_all$mean_acc[2], 2)`, SDkids = `r ms_all$st_dev_acc[2]`, Madults = `r round(ms_all$mean_acc[1], 2)`, SDadults = `r ms_all$st_dev_acc[1]`). The youngest children took longer to orient to the target (Myounger = `r round(ms_rt$mean[2],0)` ms, SDyounger = `r round(ms_rt$sd[1],0)` ms, Molder = `r round(ms_rt$mean[1],0)` ms, SDolder = `r round(ms_rt$sd[1],0)` ms, t(`r rt_tdf`) = `r rt_tval`, d = `r rt_cohd`) and were less accurate than older children (Myounger = `r round(ms_acc$mean[1],2)`, SDyounger = `r round(ms_acc$sd[1], 2)`, Molder = `r round(ms_acc$mean[2],2)`, SDolder = `r round(ms_acc$sd[2], 2)`, t(`r acc_tdf`) = `r acc_tval`, d = `r acc_cohd`). 

```{r table descriptive stats, results = 'asis'}
ms_table <- ss_all_df %>% 
    group_by(age_group) %>% 
    summarise(mean_acc = mean(target_looking_signer),
              st_dev_acc = sd(target_looking_signer))
...
ms_table <- ss_all_df %>%  
    group_by(age_group) %>% 
    summarise(mean_rt = mean(mean_correct_rt),
              st_dev_rt = sd(mean_correct_rt)) %>% 
    select(-age_group) %>% 
    bind_cols(x=ms_table)

colnames(ms_table) <- c("Age Group", "Accuracy", "SD Accuracy", "RT", "SD RT")

pander::pander(ms_table, 
               caption = "Summary statistics for all language processing measures for each age group.",
               digits = 2,
               style = "simple")
```
...
acc_age_plot <- qplot(x = Months, y = target_looking_signer, data = ss_kids_df,
      ylab=c("Accuracy"), 
    ...
    annotate("text", x = 45, y = 0.65, label = paste("r =", acc_age_r), size=8) +
    theme_bw() + 
...
rt_age_plot <- qplot(x = Months, y = mean_correct_rt, data = ss_kids_rt_df,
    ...
    geom_point(color = "black", size = 5.5) +
    geom_point(color = "grey50", size = 4) +
    annotate("text", x = 45, y = 1750, label = paste("r =", rt_age_r), size=8) +
    theme_bw() + 
    theme(axis.title.x = element_text(colour="grey30",size=22,
                                      angle=0,hjust=0.5,vjust=0,face="plain"),
          axis.title.y = element_text(colour="grey30",size=22,
...
acc_voc_plot <- qplot(x = signs_produced, y = target_looking_signer, data = ss_kids_df,
      ylab=c("Accuracy"), 
    ...
    geom_point(color = "grey50", size = 4) +
    geom_hline(yintercept = 0.33, linetype = "dashed") +
    annotate("text", x = 30, y = 0.6, label = paste("r =", acc_voc_r), size=8) +
    theme_bw() + 
    theme(axis.title.x = element_text(colour="grey30",size=22,
                                      angle=0,hjust=0.5,vjust=0,face="plain"),
          axis.title.y = element_text(colour="grey30",size=22,
...
rt_voc_plot <- qplot(x = signs_produced, y = mean_correct_rt, data = ss_kids_rt_df,
    ...
    geom_point(color = "black", size = 5.5) +
    geom_point(color = "grey50", size = 4) +
    geom_hline(yintercept = 0.33, linteype = "dashed") +
...
It is important to point out that age and vocabulary were strongly intercorrelated in our sample ($r$(`r age_voc_df`) = `r age_voc_r`). To quantify the effect of each factor on children's online language processing skills, we fit separate multiple regression models to children's accuracy and RT data (Tables 1 and 2 show model specification and output). In the accuracy model, age and vocabulary together accounted for approximately `r round(summary(fit_acc2)$r.squared, 3)*100`% of the variance in accuracy ($F$(`r round(summary(fit_acc2)$fstatistic[2])`, `r round(summary(fit_acc2)$fstatistic[3])`) = `r round(summary(fit_acc2)$fstatistic[1], 1)`). Although vocabulary did not contribute significant variance after age was taken into account ($r^2$-change: `r round(summary(fit_acc2)$r.squared, 3) * 100 - round(summary(fit_acc1)$r.squared, 3) * 100`%), age contributed approximately `r round(summary(fit_acc3)$r.squared, 3) * 100 - round(summary(fit_acc1)$r.squared, 3) * 100`% additional variance beyond vocabulary. Thus, the majority of the variation in accuracy was attributable to the shared variance between these two factors, yet some sources of individual differences in accuracy were attributable to age above and beyond vocabulary. 

```{r model output acc, results = 'asis'}
df_lm_acc <- data.frame(summary(fit_acc2)$coef)
row.names(df_lm_acc) <- c("Intercept", "Vocabulary", "Age")
colnames(df_lm_acc) <- c("Estimate", "Std. Error", "t.value", "p.value")

pander::pander(df_lm_acc, 
               caption = "Multiple regression model output for accuracy data. 
               The model was specified as: Accuracy ~ Vocabulary + Age")
```

Next, we fit the same multiple regression model to children's RT data. This analysis showed that age and vocabulary together accounted for approximately `r round(summary(fit_rt2)$r.squared, 3) * 100`% of the variance in RT. However, in contrast to the accuracy measure, vocabulary contributed significant variance even after age was included in the model ($r^2$-change: `r round(summary(fit_rt2)$r.squared, 3) * 100 - round(summary(fit_rt1)$r.squared, 3) * 100`%), age contributed approximately `r round(summary(fit_acc3)$r.squared, 3) * 100 - round(summary(fit_acc1)$r.squared, 3) * 100`

Thus, children who were faster to identify the target sign also had larger vocabularies controlling for the relationshipb between age and RT. 

```{r model output rt, results = 'asis'}
df_lm_rt <- data.frame(summary(fit_rt2)$coef)
row.names(df_lm_rt) <- c("Intercept", "Vocabulary", "Age")
colnames(df_lm_rt) <- c("Estimate", "Std. Error", "t.value", "p.value")

pander::pander(df_lm_rt, 
               caption = "Multiple regression model output for RT data. The model was specified 
               as: RT ~ Vocabulary + Age")
```
...
### Effects of hearing status

```{r age dists hearing deaf}
age_plot <- ggplot(data = ss_kids_df, aes(x=age_peek_months)) +
    geom_density(alpha=.5, position="identity") +
    facet_grid(. ~ hearing_status_participant) +
    theme_bw()
```
