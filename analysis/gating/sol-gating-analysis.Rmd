...
rm(list=ls())
source("analysis/helpers/useful.R"); library(magrittr); library(dplyr)
theme_set(theme_bw())
```
...
```{r}
df <- read.csv("analysis/gating/sol_gating_processed-df.csv", stringsAsFactors = F)
```
...
```{r}
df_final <- read.csv("analysis/gating/sol-empirical-gate-decisions.csv")
```
