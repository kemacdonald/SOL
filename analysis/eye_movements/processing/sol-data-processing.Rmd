...
write.csv(exclude_df, "../sol-exclusions.csv", row.names = F)
```
...
analysis.window_adults <- quantile(rts_adults$RT, probs=c(0.05, 0.90), na.rm=T)
...
iChart_kids <- computeStatistics(iChart_kids, startWindow=0, endWindow=5000)

# get analyisis window where 90% of RTs occur for kids, include all shifts
rts <- filter(iChart_kids, trial_types %in% c("C_T"), age_group != "Adults")

analysis.window <- quantile(rts$RT, probs=c(0.05, 0.90), na.rm=T)
...
ss_medians <- iChart %>% 
    ...
    dplyr::summarise(log_median_ct_rt = median(log(RT), na.rm = T),
                     median_ct_rt = median(RT, na.rm = T))
...
```{r, eval=T}
write.csv(ss, "../sol_ss_all.csv", row.names = F)
```
...
write.csv(x = ms_mean_iChart, file = "../sol_gvals_all.csv", row.names = F)
```
...
```{r, echo=F, eval=F, message=F, warning=F}
write.csv(x = ms_mean_iChart_coda, file = "../sol_gvals_coda.csv", row.names = F)
```
