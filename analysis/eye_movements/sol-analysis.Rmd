...
acc <- poolData(meanAccuracy(iChart_kids, startWindowAcc=analysis.window[1], endWindowAcc=analysis.window[2]), 
                RejectFirstGap=TRUE,RejectLongestGap=TRUE, 
                RejectRT=FALSE, color=TRUE, dependent="Accuracy", 
...
Hmisc::rcorr(as.matrix(select(ss_kids, mean_accuracy, C_T_prop, 
                              mean_correct_rt, median_ct_rt, signs_produced, 
                              Months)))

ggpairs(data = select(ss_kids, mean_accuracy, C_T_prop, signs_produced, Months),
...
```{r}
qplot(C_T_prop, mean_accuracy, data = ss_kids) +
    geom_abline(intercept = 0, slope = 1) +
    xlim(0,1) +
    ylim(0,1) +
    geom_text(aes(label = Sub.Num), position = "dodge")
```

Just RT.

```{r, fig.height=8, fig.width=10, warning = F}
Hmisc::rcorr(as.matrix(select(ss_rt, mean_correct_rt, median_ct_rt, signs_produced, Months)))

ggpairs(data = select(ss_rt, mean_correct_rt, median_ct_rt, signs_produced, Months),
        upper = list(continuous = "smooth", combo = "cor", 
                     params = c(method = "lm", color="darkblue")),
        lower = list(continuous = "cor"),
        diag = list(continuous = "density")) +
...
```{r t-tests age}
t.test(mean_correct_rt ~ age_group, alternative = "two.sided", 
                   var.equal=T, data = ss_rt)

t.test(mean_accuracy ~ age_group, alternative = "two.sided", 
                   var.equal=T, data = ss_kids)

t.test(mean_correct_rt ~ age_group_collapsed, alternative = "two.sided",
       var.equal = T, data = ss)
```

```{r t-tests hearing status}
t.test(mean_accuracy ~ hearing_status_participant, alternative = "two.sided", 
                   var.equal=T, data = ss_kids)

t.test(C_T_prop ~ hearing_status_participant, alternative = "two.sided", 
                   var.equal=T, data = ss_kids)

t.test(mean_correct_rt ~ hearing_status_participant, alternative = "two.sided", 
                   var.equal=T, data = ss_kids)
```
...
qplot(data = ms_graph_values, x = Time.ms, y = mean_accuracy,
      color=age_group, geom = "blank", xlab=c("Time in ms from onset of noun"), 
      ylab=c("ACCURACY\nProportion\nlooking\nto target")) + 
...
ms_all <- ss %>% 
    ...
    dplyr::summarise(m_age = mean(age_peek_months),
              ...
              m_acc = mean(mean_accuracy),
              ci.h_acc = ci.high(mean_accuracy),
              ci.l_acc = ci.low(mean_accuracy))
...
gridExtra::grid.arrange(all_fs, all_acc, all_rt, ncol = 1)
```
...
```{r}
ss_iChart <- iChart %>% 
    select(Sub.Num, age_group, Response, `0`:`4000`) %>% 
    filter(Response == "D") 
```
...
gridExtra::grid.arrange(codas_deaf_fs, codas_deaf_acc, codas_deaf_rt, ncol = 1)
```
...
breaks <- seq(0,4000, by=500)
y_breaks <- seq(0.25,1.0, by = 0.25)
points <- seq(0,4000, by = 100)

ggplot(aes(x = to.n(Time.ms), y = mean_prop_looking, linetype = hearing_status_participant, 
           shape = hearing_status_participant, color = value_cat), 
       data = ms_mean_iChart_coda) + 
    ylim(0,1) +
    geom_point(data = filter(ms_mean_iChart_coda, to.n(Time.ms) %in% points), size=3) +
    geom_line(data = filter(ms_mean_iChart_coda, Time.ms %in% points), size=0.7) +
...
```{r plot correlations}
acc_age_plot <- qplot(x = Months, y = mean_accuracy, data = ss_kids,
      ylab=c("Accuracy"), 
...
acc_voc_plot <- qplot(x = signs_produced, y = mean_accuracy, data = ss_kids,
      ylab=c("Accuracy"), 
...
rt_voc_plot <- qplot(x = signs_produced, y = mean_correct_rt, data = ss_rt,
      ylab=c("Reaction Time (ms)"), 
      xlab=c("Signs Produced")) +
    ylim(800, 1950) +
    xlim(15, 85) +
...
gridExtra::grid.arrange(acc_age_plot, rt_age_plot, acc_voc_plot, rt_voc_plot, ncol = 2)
