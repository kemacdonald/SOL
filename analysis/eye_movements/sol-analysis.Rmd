...
df_corrs <- ss_kids %>% 
    ...
    select(Sub.Num, prop_looking, age_cdi_months, signs_produced, mean_correct_rt)

df_corrs_rt <- ss_rt %>% 
    filter(value_cat == "Target") %>% 
    select(Sub.Num, prop_looking, age_cdi_months, signs_produced, mean_correct_rt)

acc_age_r <- round(cor.test(df_corrs$prop_looking, df_corrs$age_cdi_months)$estimate, 2)
rt_age_r <- round(cor.test(df_corrs_rt$mean_correct_rt, df_corrs_rt$age_cdi_months)$estimate, 2)
acc_voc_r <- round(cor.test(df_corrs$prop_looking, df_corrs$signs_produced)$estimate, 2)
rt_voc_r <- round(cor.test(df_corrs_rt$mean_correct_rt, df_corrs_rt$signs_produced)$estimate, 2)
...
rt_age_plot <- qplot(x = Months, y = mean_correct_rt, 
                     data = filter(ss_kids, value_cat == "Target"),
...
rt_voc_plot <- ggplot(aes(x = signs_produced, y = mean_correct_rt), 
...
fit.2 <- lm(mean_prop_looking_TD ~ age_group * value_cat, 
            data = filter(td_looking, age_group == "Kids"))
```