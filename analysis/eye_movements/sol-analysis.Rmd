...
ss_rt <- filter(ss, age_group != "Adults")

ss_kids %>% 
    filter(exclude_chance_shifter == "exclude") %>% 
    select(Sub.Num, age_cdi_months) %>% 
    unique()
...
ms_coda_deaf <- ss_kids %>% 
    ...
    dplyr::summarise(m_age = mean(age_peek_months),
              ...
              m_acc = mean(prop_looking),
              ci.h_acc = ci.high(prop_looking),
              ci.l_acc = ci.low(prop_looking),
...
df_corrs <- ss_kids %>% 
    ...
    select(Sub.Num, prop_looking, Months, signs_produced, median_ct_rt)

df_corrs_rt <- ss_rt %>% 
    filter(value_cat == "Target", age_group != "Adults") %>% 
    select(Sub.Num, prop_looking, Months, signs_produced, median_ct_rt)

acc_age_r <- round(cor.test(df_corrs$prop_looking, df_corrs$Months)$estimate, 2)
rt_age_r <- round(cor.test(df_corrs_rt$median_ct_rt, df_corrs_rt$Months)$estimate, 2)
acc_voc_r <- round(cor.test(df_corrs$prop_looking, df_corrs$signs_produced)$estimate, 2)
rt_voc_r <- round(cor.test(df_corrs_rt$median_ct_rt, df_corrs_rt$signs_produced)$estimate, 2)

acc_age_plot <- qplot(x = Months, y = prop_looking, 
                      data = df_corrs,
      ylab=c("Accuracy"), 
    ...
    annotate("text", x = 45, y = 0.7, label = paste("r =", acc_age_r), size=8) +
    theme_bw() + 
...
acc_voc_plot <- qplot(x = signs_produced, y = prop_looking, 
                      data = df_corrs,
                      ylab=c("Accuracy"), 
    ...
    geom_point(color = "black", size = 5.5) +
    geom_point(color = "grey50", size = 4) +
    annotate("text", x = 25, y = 0.6, label = paste("r =", acc_voc_r), size=8) +
    theme_bw() + 
    theme(axis.title.x = element_text(colour="grey30",size=22,
                                      angle=0,hjust=0.5,vjust=0,face="plain"),
          axis.title.y = element_text(colour="grey30",size=22,
...
rt_voc_plot <- ggplot(aes(x = signs_produced, y = median_ct_rt), 
                      data = df_corrs) + 
...
```{r}
fit_rt1 <- lm(median_ct_rt ~ signs_produced, data = df_corrs)
fit_rt2 <- lm(median_ct_rt ~ signs_produced + Months, data = df_corrs)
fit_rt3 <- lm(median_ct_rt ~ Months, data = df_corrs)
```
...
### Model target and distractor looking as a function of age 

```{r}
td_looking <- filter(ss, value_cat %in% c("Target", "Distractor"))
tds_looking <- filter(ss, value_cat %in% c("Target", "Distractor", "Signer"))

fit.1 <- lm(prop_looking ~ age_group * value_cat, data = tds_looking)
```