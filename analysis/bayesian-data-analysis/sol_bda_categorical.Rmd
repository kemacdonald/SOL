...
knitr::opts_chunk$set(fig.height=4, fig.width=7, cache = T,
                      warning=F, message=F, results = "hide")

library(langcog); library(knitr); library(dplyr); library(ggplot2)
library(tidyr); library(polspline); library(rethinking); library(forcats);
library(magrittr); library(stringr); theme_set(theme_bw())
...
all_acc <- ggplot(aes(x = age_group_fact, y = mean_post, fill = looking_type), 
  ...
  labs(x = "Age Group", y = "Proportion Looking", title = "Accuracy") + 
  coord_cartesian(ylim=c(0, 1)) +
  scale_fill_grey(start = 0, breaks = c("Target", "Distracter")) +
...
all_rt <- ggplot(aes(x= age_group_fact, y = mean_post), 
  ...
  labs(x = "Age Group", y = "Mean RT (ms)", title = "Reaction Time") +
  scale_x_discrete(labels=c("Younger", "Older", "Adults")) +
  ylim(0, 1600) +
  theme(plot.title = element_text(face = "bold", size = 20),
        axis.title.x = element_text(colour="grey40",size=18,
