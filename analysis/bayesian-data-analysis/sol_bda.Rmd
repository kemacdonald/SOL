...
In this document, you will find the details of our Bayesian analysis for the paper, "Real-time Language Comprehension in American Sign Language." Note that numerical values will be slightly different each time because of simulation variance.
...
* RT estimate: is 50ms change with each month of development
...
### Bayes Factor for CODA vs. Deaf comparison

From @lee2014bayesian Ch.8 on two-sample comparisons. 
...
samples <- jags(data = dataList_age, parameters.to.save = parameters,
                      model.file = "models/accuracy_model.txt", n.chains=nChains, 
                      n.iter=nIter, n.burnin = burnInSteps,
                      n.thin=1, DIC=F)

df_acc_age <- data.frame(zbeta0 = samples$BUGSoutput$sims.list$zbeta0, 
...
df_acc_age %<>% 
    mutate( beta1 = zbeta1 * sd(d$mean_prop_looking_TD) / sd(d$age_peek_months),
            beta1_prior = zbeta1_prior * sd(d$mean_prop_looking_TD) / sd(d$age_peek_months),
            ...
            beta0_prior = zbeta0_prior * sd(d$mean_prop_looking_TD) + mean(d$mean_prop_looking_TD) - (zbeta1_prior * mean(d$age_peek_months) * sd(d$mean_prop_looking_TD)) / sd(d$age_peek_months)
    )
```
...
```{r}
a <- ggplot() + geom_density(aes(df_acc_age$beta1_prior)) +
    xlab("Beta1") +
    ylab("Density") +
    ggtitle("Prior Predictive on Slope")

b <- ggplot() + geom_density(aes(df_acc_age$beta0)) +
    xlab("Beta0") +
    ylab("Density") +
    ggtitle("Prior Predictive on Intercept")
...
```{r}
ggplot(data=df_acc_age) +
    geom_density(aes(x=zbeta1), linetype='dashed') +
    geom_density(aes(x=zbeta1_prior), data = df_acc_age) +
    xlim(0,2) +
    xlab("Standardized Beta")
```

We can see that the prior model puts most of the density on a value of 0, or a null relationship between age and accuracy. It is truncated at zero to eoncode our directional hypothesis: that the slope parameter should be positive. And the prior does not consider extreme slope values beyond .04, or a 4% gain in accuracy with each month. 

Now that we are confident that our prior distribution produces reasonable hypotheses, we can quantify the association between children's age and accuracy on the VLP task. 
...
# uses 1997 knot and deletion algorithm
fit.posterior <- logspline(df_acc_age$zbeta1, lbound = 0)
posterior <- dlogspline(0, fit.posterior) # pdf @ beta=0

# get prior density using logspline
fit.prior <- logspline(df_acc_age$zbeta1_prior, lbound = 0)
prior <- dlogspline(0, fit.prior) # pdf @ beta=0
```

We get the Bayes Factor using the Savage-Dickey moethod as described in @wagenmakers2010bayesian, which takes the ratio of the density of the posterior and prior when the slope parameter is zero. 
...
```{r}
post_mode_b1_acc_age <- round(find_mode(df_acc_age$beta1), 3)
post_mode_b0_acc_age <- round(find_mode(df_acc_age$beta0), 3)
HDI_b1_acc_age <- HDIofMCMC(df_acc_age$beta1 , credMass = 0.95 )

## simulate posterior distribution of mu for each value of age to get HDI for MAP regression line
mu.link <- function(x) {df_acc_age$beta0 + df_acc_age$beta1*x}
...
samples <- jags(data = dataList_acc_voc, parameters.to.save = parameters,
                      model.file = "models/accuracy_model.txt", n.chains=nChains, 
                      n.iter=nIter, n.burnin = burnInSteps,
                      n.thin=1, DIC=T)

df_acc_voc <- data.frame(zbeta0 = samples$BUGSoutput$sims.list$zbeta0, 
                       zbeta1 = samples$BUGSoutput$sims.list$zbeta1,
                       zbeta0_prior = samples$BUGSoutput$sims.list$zbeta0_prior,
                       zbeta1_prior = samples$BUGSoutput$sims.list$zbeta1_prior)
...
df_acc_voc %<>% 
    mutate( beta1 = zbeta1 * sd(d_voc$mean_prop_looking_TD) / sd(d_voc$signs_produced),
            beta1_prior = zbeta1_prior * sd(d_voc$mean_prop_looking_TD) / sd(d_voc$signs_produced),
            ...
            beta0_prior = zbeta0_prior * sd(d_voc$mean_prop_looking_TD) + mean(d_voc$mean_prop_looking_TD) - (zbeta1_prior * mean(d_voc$signs_produced) * sd(d_voc$mean_prop_looking_TD)) / sd(d_voc$signs_produced)
    )
```
...
```{r}
ggplot(data=df_acc_voc) +
    geom_density(aes(x=zbeta1), linetype='dashed') +
    geom_density(aes(x=zbeta1_prior), data = df_acc_voc) +
    xlim(0,2) +
    xlab("Standardized Beta")
```
...
# get prior density using logspline
fit.prior <- logspline(df_acc_voc$beta1_prior, lbound = 0)
prior <- dlogspline(0, fit.prior) # pdf @ beta=0
...
samples <- jags(data = dataList_age_rt, parameters.to.save = parameters,
                model.file = "models/rt_model.txt", n.chains=nChains, 
                n.iter=nIter, n.burnin = burnInSteps, inits = myinits,
                n.thin=1, DIC=T)


df_rt_age <- data.frame(zbeta0 = samples$BUGSoutput$sims.list$true_beta0,
...
df_rt_age %<>% mutate(
    ...
    beta1_prior = zbeta1_prior * sd(d$median_rt) / sd(d$age_peek_months),
    beta0 = zbeta0 * sd(d$median_rt) + mean(d$median_rt) - zbeta1 * mean(d$age_peek_months) * sd(d$median_rt) / sd(d$age_peek_months),
    beta0_prior = zbeta0_prior * sd(d$median_rt) + mean(d$median_rt) - zbeta1_prior * mean(d$age_peek_months) * sd(d$median_rt) / sd(d$age_peek_months)
)
```

First, we plot samples from the prior predictive distribution to show that we are considering 
a plausible set of parameter values for the slope and interecept. Here we use past research on spoken language to inform our prior: that expected change in RT for each month of age is ~50ms.

```{r}
ggplot() + geom_density(aes(df_rt_age$beta1_prior)) +
    xlab("Beta 1") +
    ylab("Density") +
...
and RT. The value from prior data of 50 ms is considered plausible, and extends to extreme values of triple that value.

Plot prior with posterior.

```{r}
ggplot(data=df_rt_age) +
    geom_density(aes(x=zbeta1), linetype='dashed') +
    geom_density(aes(x=zbeta1_prior), data = df_rt_age) +
    xlim(-1, 0) +
    xlab("Standardized Beta")
```
...
# Fits a density using splines to approx. log-density
# uses 1997 knot and deletion algorithm
fit.posterior <- logspline(df_rt_age$zbeta1, ubound = 0)
posterior <- dlogspline(0, fit.posterior) # pdf @ beta=0

# get prior density using logspline
fit.prior <- logspline(df_rt_age$zbeta1_prior, ubound = 0)
prior <- dlogspline(0, fit.prior) # pdf @ beta=0
...
Draw samples from prior and posterior.

```{r}
dataList_voc_rt= list(
    x = d_voc$voc.s,
    y = d_voc$rt.s,
...
samples <- jags(data = dataList_voc_rt, parameters.to.save = parameters,
                model.file = "models/rt_model.txt", n.chains=nChains, inits = myinits,
                n.iter=nIter, n.burnin = burnInSteps,
                n.thin=1, DIC=F)


df_rt_voc <- data.frame(zbeta0 = samples$BUGSoutput$sims.list$true_beta0,
                        zbeta1 = samples$BUGSoutput$sims.list$true_beta1,
                        zbeta0_prior = samples$BUGSoutput$sims.list$true_beta0_prior,
                        zbeta1_prior = samples$BUGSoutput$sims.list$true_beta1_prior,
                        z = samples$BUGSoutput$sims.list$z)
...
df_rt_voc%<>% mutate(
    ...
    beta1_prior = zbeta1_prior * sd(d_voc$median_rt) / sd(d_voc$signs_produced),
    beta0_prior = zbeta0_prior * sd(d_voc$median_rt) + mean(d_voc$median_rt) - zbeta1_prior * mean(d_voc$signs_produced) * sd(d_voc$median_rt) / sd(d_voc$signs_produced)
)
```
...
```{r}
ggplot(data=df_rt_voc) +
    geom_density(aes(x=beta1), linetype='dashed') +
    geom_density(aes(x=beta1_prior), data = df_rt_voc) +
    xlim(-20, 0) +
    xlab("RT Beta")
```
...
# Fits a density using splines to approx. log-density
# uses 1997 knot and deletion algorithm
fit.posterior <- logspline(df_rt_voc$zbeta1, ubound = 0)
posterior <- dlogspline(0, fit.posterior) # pdf @ beta=0

# get prior density using logspline
fit.prior <- logspline(df_rt_voc$zbeta1_prior, ubound = 0)
prior <- dlogspline(0, fit.prior) # pdf @ beta=0
...
samples <- jags(data = dataList_acc_rt, parameters.to.save = parameters,
                      model.file = "models/rt_model_nomixture.txt", n.chains=nChains, 
                      n.iter=nIter, n.burnin = burnInSteps,
                      n.thin=1, DIC=T)

df_rt_acc <- data.frame(zbeta0 = samples$BUGSoutput$sims.list$zbeta0, 
                       zbeta1 = samples$BUGSoutput$sims.list$zbeta1,
                       zbeta0_prior = samples$BUGSoutput$sims.list$zbeta0_prior,
                       zbeta1_prior = samples$BUGSoutput$sims.list$zbeta1_prior)
...
## transform to measurement scale (RT in ms)
df_rt_acc %<>% mutate(
    beta1 = zbeta1 * sd(d$median_rt) / sd(d$mean_prop_looking_TD),
    beta1_prior = zbeta1_prior * sd(d$median_rt) / sd(d$mean_prop_looking_TD),
    beta0 = zbeta0 * sd(d$median_rt) + mean(d$median_rt) - zbeta1 * mean(d$mean_prop_looking_TD) * sd(d$median_rt) / sd(d$mean_prop_looking_TD),
    beta0_prior = zbeta0_prior * sd(d$median_rt) + mean(d$median_rt) - zbeta1_prior * mean(d$mean_prop_looking_TD) * sd(d$median_rt) / sd(d$mean_prop_looking_TD)
)
```
...
```{r}
ggplot() + geom_density(aes(df_rt_acc$beta1_prior)) +
    xlab("Beta 1") +
    ylab("Density") +
    ggtitle("Prior Predictive on Slope Parameter")
```
...
```{r}
ggplot(data=df_rt_acc) +
    geom_density(aes(x=zbeta1), linetype='dashed') +
    geom_density(aes(x=zbeta1_prior), data = df_rt_acc) +
    xlim(-1, 0) +
    xlab("Standardized Beta")
```
...
# Fits a density using splines to approx. log-density
# uses 1997 knot and deletion algorithm
fit.posterior <- logspline(df_rt_acc$zbeta1, ubound = 0)
posterior <- dlogspline(0, fit.posterior) # pdf @ beta=0

# get prior density using logspline
fit.prior <- logspline(df_rt_acc$zbeta1_prior, ubound = 0)
prior <- dlogspline(0, fit.prior) # pdf @ beta=0

bf_rt_acc <- posterior / prior
1/bf_rt_acc
```

```{r}
HDI_rt_acc_b1 <- HDIofMCMC(df_rt_acc$beta1 , credMass = 0.95 )
post_rt_acc_mode_b1 <- round(mean(df_rt_acc$beta1), 3)
post_rt_acc_mode_b0 <- round(mean(df_rt_acc$beta0), 3)

# simulate to get HDI around MAP regression line
mu.link <- function(x) {df_rt_acc$beta0 + df_rt_acc$beta1*x}
acc.seq <- seq(from = 0.2, to = 0.9, by = 0.1)
mu <- sapply(acc.seq, mu.link)
mu.mean <- apply(mu, 2, mean)
mu.HPDI <- apply(mu, 2, HPDI, prob = 0.95)
mu.HPDI_tidy <- data.frame(mean_prop_looking_TD = acc.seq, 
                           hpdi_lower = mu.HPDI[1,], hpdi_upper = mu.HPDI[2,],
                           mu.rt = mu.mean)

ggplot(data = d) +
    geom_line(aes(x = mean_prop_looking_TD, y = mu.rt), data = mu.HPDI_tidy, size = 2) +
    ...
    ylab("Mean Reaction Time (ms)") +
    xlab("Mean Accuracy") +
    coord_cartesian(xlim=c(0.2, 0.9), ylim=c(700, 2000)) +
    ggtitle(bquote(list(beta==.(as.character(round(post_rt_acc_mode_b1, 2))),
                        HDI=.(paste("95% HDI [", round(HDI_rt_acc_b1[1],3) , "," , 
                                    round(HDI_rt_acc_b1[2], 3), "]"))))) +
    theme(plot.title = element_text(size = 18),
        axis.title.x = element_text(colour="grey30",size=22,
                                      angle=0,hjust=0.5,vjust=0,face="plain"),
          axis.title.y = element_text(colour="grey30",size=22,
...
### Age and RT no latent mixture

Draw samples from prior and posterior.

```{r}
# Specify the data in a list, for later shipment to JAGS:
dataList_age_rt_nolat = list(
    x = d$age.s,
    y = d$rt.s,
    Ntotal = length(d$acc.s)
)

parameters = c( "zbeta0", "zbeta1", "zbeta0_prior", "zbeta1_prior")

samples <- jags(data = dataList_age_rt_nolat, parameters.to.save = parameters,
                model.file = "models/rt_model_nomixture.txt", n.chains=nChains, 
                n.iter=nIter, n.burnin = burnInSteps,
                n.thin=1, DIC=F)


df_rt_age_nolat <- data.frame(zbeta0 = samples$BUGSoutput$sims.list$zbeta0,
                              zbeta1 = samples$BUGSoutput$sims.list$zbeta1,
                              zbeta0_prior = samples$BUGSoutput$sims.list$zbeta0_prior,
                              zbeta1_prior = samples$BUGSoutput$sims.list$zbeta1_prior)

## transform to measurement scale (RT in ms)
df_rt_age_nolat %<>% mutate(
    beta1 = zbeta1 * sd(d$median_rt) / sd(d$age_peek_months),
    beta0 = zbeta0 * sd(d$median_rt) + mean(d$median_rt) - zbeta1 * mean(d$age_peek_months) * sd(d$median_rt) / sd(d$age_peek_months),
    beta1_prior = zbeta1_prior * sd(d$median_rt) / sd(d$age_peek_months),
    beta0_prior = zbeta0_prior * sd(d$median_rt) + mean(d$median_rt) - zbeta1_prior * mean(d$age_peek_months) * sd(d$median_rt) / sd(d$age_peek_months)
...
Plot prior with posterior for slope parameter

```{r}
ggplot(data=df_rt_age_nolat) +
    geom_density(aes(x=zbeta1), linetype='dashed') +
    geom_density(aes(x=zbeta1_prior), data = df_rt_age_nolat) +
    xlim(-1, 0) +
    xlab("Standardized Beta")
```
...
# Fits a density using splines to approx. log-density
# uses 1997 knot and deletion algorithm
fit.posterior <- logspline(df_rt_age_nolat$zbeta1, ubound = 0)
posterior <- dlogspline(0, fit.posterior) # pdf @ beta=0

# get prior density using logspline
fit.prior <- logspline(df_rt_age_nolat$zbeta1_prior, ubound = 0)
prior <- dlogspline(0, fit.prior) # pdf @ beta=0

bf_rt_age_nolat <- posterior / prior
1/bf_rt_age_nolat
```
...
```{r}
HDI_rt_age_b1 <- HDIofMCMC(df_rt_age_nolat$beta1 , credMass = 0.95 )
post_rt_age_mode_b1 <- round(find_mode(df_rt_age_nolat$beta1), 3)
post_rt_age_mode_b0 <- round(find_mode(df_rt_age_nolat$beta0), 3)

# simulate to get HDI around MAP regression line
mu.link <- function(x) {df_rt_age_nolat$beta0 + df_rt_age_nolat$beta1*x}
age.seq <- seq(from = 10, to = 60, by = 1)
mu <- sapply(age.seq, mu.link)
mu.mean <- apply(mu, 2, mean)
mu.HPDI <- apply(mu, 2, HPDI, prob = 0.95)
...
### Vocab and RT no latent mixture

Fit prior and posterior. 

```{r}
# Specify the data in a list, for later shipment to JAGS:
dataList_voc_rt_nolat = list(
    x = d_voc$voc.s,
    y = d_voc$rt.s,
    Ntotal = length(d_voc$voc.s)
)

samples <- jags(data = dataList_voc_rt_nolat, parameters.to.save = parameters,
                model.file = "models/rt_model_nomixture.txt", n.chains=nChains, 
                n.iter=nIter, n.burnin = burnInSteps,
                n.thin=1, DIC=F)


df_rt_voc_nolat <- data.frame(zbeta0 = samples$BUGSoutput$sims.list$zbeta0,
                              zbeta1 = samples$BUGSoutput$sims.list$zbeta1,
                              zbeta0_prior = samples$BUGSoutput$sims.list$zbeta0_prior,
                              zbeta1_prior = samples$BUGSoutput$sims.list$zbeta1_prior)

## transform to measurement scale (RT in ms)
df_rt_voc_nolat %<>% mutate(
    beta1 = zbeta1 * sd(d_voc$median_rt) / sd(d_voc$signs_produced),
    beta0 = zbeta0 * sd(d_voc$median_rt) + mean(d_voc$median_rt) - zbeta1 * mean(d_voc$signs_produced) * sd(d_voc$median_rt) / sd(d_voc$signs_produced),
    beta1_prior = zbeta1_prior * sd(d_voc$median_rt) / sd(d_voc$signs_produced),
    beta0_prior = zbeta0_prior * sd(d_voc$median_rt) + mean(d_voc$median_rt) - zbeta1_prior * mean(d_voc$signs_produced) * sd(d_voc$median_rt) / sd(d_voc$signs_produced)
...
```{r}
ggplot(data=df_rt_voc_nolat) +
    geom_density(aes(x=zbeta1), linetype='dashed') +
    geom_density(aes(x=zbeta1_prior), data = df_rt_voc_nolat) +
    xlim(-1, 0) +
    xlab("Standardized Beta")
```
...
# Fits a density using splines to approx. log-density
# uses 1997 knot and deletion algorithm
fit.posterior <- logspline(df_rt_voc_nolat$zbeta1, ubound = 0)
posterior <- dlogspline(0, fit.posterior) # pdf @ beta=0

# get prior density using logspline
fit.prior <- logspline(df_rt_voc_nolat$zbeta1_prior, ubound = 0)
prior <- dlogspline(0, fit.prior) # pdf @ beta=0

bf_rt_voc_nolat <- posterior / prior
1/bf_rt_voc_nolat
```
...
```{r}
HDI_rt_voc_b1 <- HDIofMCMC( df_rt_voc_nolat$beta1 , credMass = 0.95 )
post_mode_b1_rt_voc <- round(find_mode(df_rt_voc_nolat$beta1), 3)
post_mode_b0_rt_voc <- round(find_mode(df_rt_voc_nolat$beta0), 3)

vocab.seq <- seq(from = 15, to = 85, by = 1)
mu.link <- function(x) {df_rt_voc_nolat$beta0 + df_rt_voc_nolat$beta1*x}
mu <- sapply(vocab.seq, mu.link)
mu.mean <- apply(mu, 2, mean)
mu.HPDI <- apply(mu, 2, HPDI, prob = 0.95)
mu.HPDI_tidy <- data.frame(signs_produced = vocab.seq,
                           hpdi_lower = mu.HPDI[1,], 
                           hpdi_upper = mu.HPDI[2,],
                           mu.rt = mu.mean)
...
samples_post <- jags(data = dataList_lat_mix, parameters.to.save = parameters,
                model.file = "models/latent_mixture.txt", n.chains=nChains, inits = myinits,
                n.iter=nIter, n.burnin = burnInSteps,
                n.thin=1, DIC=T)
...
```{r}
df_latent_mix <- select(df_rt_age, z.1:z.29)
...
```{r}
df_latent_mix <- select(df_rt_voc, z.1:z.28)
...
The estimates are relatively stable across the different models. Most participants are not likely to be guessing. Four participants have a high probability of guessing across all models and are thus less likely to contribute to the linear regression. 30050 is more likely to be guessing in the models with the linear regressions.
