...
```{r}
# list of gate names
gate_list <- c("ball_find", "ball_where_1", "ball_where_4", "ball_which",
               "bear_look", "bear_where_1", "bear_where_2", 
               "bear_where_4","bear_which",
               "bird_look", "bird_where_1", "bird_where_2", "bird_where_4", 
               "book_look", "book_where_2", "book_where_4", "book_which",
               "car_where_1", "car_where_3", "car_which", 
               "cat_where", "cat_which", 
               "doll_find", "doll_where_1", "doll_where_3", 
               "doll_where_4","doll_which",
               "kitty_hey", "kitty_where",
               "love_ball", "love_bear", "love_bird", "love_book",
               "love_car", "love_cat" ,"love_doll", "love_shoes", 
               "shoe_hey", "shoe_where", "shoes_where", "shoes_which")

# extract string from long ugly gate name
gates <- sapply(all.data$gate, function (x) str_extract(x, gate_list))
...
Check to make sure we have six gates for each sign
...
```{r}
write.csv(all.data, file = "sol-gating-processed-df.csv", row.names=F)
```
