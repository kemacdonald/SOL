...
```{r}
read.path <- "../../data/processed_data/"

# Demographic data
sol_demo <- read.csv(paste0(read.path, "summary_tables/sol_demo_all.csv"), 
                     stringsAsFactors = F) %>% 
  mutate(Sub.Num = as.character(Sub.Num))

sol_demo$Sub.Num <- as.character(sol_demo$Sub.Num)

iChart <- read.csv(paste0(read.path, "vanilla/sol-ichart-merged-post-gating.csv"), 
                   check.names=F, stringsAsFactors=F) %>%  
  mutate(Sub.Num =  as.character(Sub.Num))

# Stimuli information
sol_target_signs_df <- read.csv(paste0(read.path,"summary_tables/target_sign_lengths.csv"))
```
...
ss_unknown_signs <- sol_demo %>% 
  ...
  mutate(parent_report_unknown_signs = ifelse(is.na(parent_report_unknown_signs) == T, "none",
                                              parent_report_unknown_signs))
...
ss_trials_df %<>% 
  mutate(num_prescreened = as.numeric(num_prescreened),
         num_prescreened = ifelse(is.na(num_prescreened) == T, 0, num_prescreened),
...
ggplot(aes(RT), data = rts_adults) +
  ...
  geom_vline(xintercept = analysis.window_adults[1], col="red", lwd=1.5, linetype = "dashed") +
  geom_vline(xintercept = analysis.window_adults[2], col="red", lwd=1.5, linetype = "dashed") +
...
```{r, eval=F}
write.csv(ss, "../../data/processed_data/summary_tables/sol_ss_all_600_2500.csv", row.names = F)
```
...
write.csv(x = ms_mean_iChart, 
          file = "../../data/processed_data/summary_tables/sol_gvals_all.csv", 
          row.names = F)
```
...
write.csv(x = ms_mean_iChart_coda, 
          file = "../../data/processed_data/summary_tables/sol_gvals_coda.csv", 
          row.names = F)
```
...
ss_trial_level_df <- iChart %>% 
  select(Sub.Num, Sex, Response, Tr.Num, RT, Accuracy, signs_produced, age_code,
