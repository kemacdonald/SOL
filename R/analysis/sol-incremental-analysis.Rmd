...
library(magrittr); library(tidyverse); library(rethinking)
...
```{r}
ss.all.signs %>% 
  ggplot(., aes(x = m_prop_sign_needed)) +
  geom_histogram()
```

Remove outliers +/- 2 sd from the mean for each group
...
Plot distribution, flagging outliers

```{r}
ss.all.signs %>% 
  ggplot(., aes(x = m_prop_sign_needed, fill = age_code)) +
...
```{r}
ms.final <- bind_rows(ms, ms_all_signs) %>% 
...
Clean up variable names and order for plot

```{r}
ms.final$age_code <- recode(ms.final$age_code, child = "children", adult = "adults")
ms.final$age_code <- fct_relevel(ms.final$age_code, "children")
```
...
item_plot <- ms.final %>% 
  ...
  labs(y = "Prop. Target Sign Prior to Gaze Shift", x = "Target sign") +
...
Get summary information about sign lengths
