...
library(magrittr); library(tidyverse); theme_set(theme_minimal())
```
...
m1 <- d_2500 %>% 
  filter(age_group != "Adults", value_cat == "Target") %>% 
  select(Sub.Num, prop_looking, age_peek_months, signs_produced, hearing_status_participant) %>%
  unique() %>% 
  lm(prop_looking ~ age_peek_months + hearing_status_participant, data = .)

summary(m1)$coef %>% 
  mutate(one_sided_p = pt())
  kable(digits = 3)
```
...
m2 <- d_2500 %>% 
  filter(age_group != "Adults", value_cat == "Target") %>% 
  select(Sub.Num, median_ct_rt, age_peek_months, signs_produced, hearing_status_participant) %>% 
  unique() %>% 
