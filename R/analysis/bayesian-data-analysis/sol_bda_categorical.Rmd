...
d_model <- d %>% 
  ...
  select(Sub.Num, age_group_collapsed, hearing_status_participant, mean_prop_looking_TD, median_ct_rt) %>% 
...
post_df_hearing_stat.acc <- data.frame(hearing, deaf) %>% 
  ...
  mutate(age_group = "kids",
         measure = "accuracy",
         prob_acc = logit2prob(posterior_sample))


post_df_hearing_stat.acc %>% 
  group_by(hearing_status) %>% 
  summarise(m = round(mean(prob_acc), 2))
...
post_hearing_stat.rt_df <- data.frame(hearing, deaf) %>% 
  gather(key = hearing_status, value = posterior_sample) %>% 
  mutate(age_group = "kids",
         measure = "rt")

post_hearing_stat.rt_df %>% 
  group_by(hearing_status) %>% 
